...
assertion_fact(
 ...
 pp_e(join(A,B,_)) :- unary_relation(B),!, % this should be provided by the type checker
...
is_field(Set,Name,Type) :- signature(this/Set,Fields,_,_,_),
...
pp_model :-
    is_field(Set,Name,Type), format(' ~w : ~w <-> ',[Name,Set]), pp_t(Type),nl,fail.
